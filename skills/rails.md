...
2. `{{REPO_PATH}}/docs/skills/rails.md` (if it exists) — extensions
   ...
   `AGENTS.md` (e.g. test-suite layout, component conventions
   ViewComponent / Phlex / helpers, ERB style, Stimulus controller
   naming, system-spec scaffolding).

If `docs/skills/rails.md` is absent, that's fine — only the
...
project-scoped, not skill-scoped. Honor it from the two docs above.
...
project agent/skill configs. The root `Cargo.toml` (workspace manifest)
is also off-limits — it is owned by whoever modifies the workspace
member list.
...
3. The in-repo top-level reference docs the project's `AGENTS.md` lists. They
...
- Stimulus controllers under `app/javascript/controllers/` for any new web
  ...
  project's `AGENTS.md` explicitly authorizes them.
- ERB views (or whatever view layer the project's `AGENTS.md` declares).
...
- The user safeguards this folder with git commits. Inside this folder you may
  ...
  `docs/`, or project agent/skill configs); outside the folder, you ask first.
