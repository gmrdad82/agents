#!/usr/bin/env bash
# install-skills.sh — concatenate selected skills into <project>/AGENTS.md
#
# Usage:
...
#
# Modes:
#   update    (default) — replace existing skill blocks with current source,
#                         add any new skills from --include, regenerate the
#                         banner and TOC. Leaves any non-marker content
#                         (custom preamble, etc.) untouched.
#   append    — only add skills not already present in <project>/AGENTS.md.
#                Never overwrites an existing skill block.
#   override  — rebuild <project>/AGENTS.md from scratch (destructive).
#
# Nothing is written outside <path/to/project>. Source skills live in
# <repo>/skills/ alongside this script's parent directory.
#
# Note: the output file is named AGENTS.md (Claude Code / OpenCode load
# agent prompts from that fixed name). Inside it, each skill is wrapped
# in <!-- agents:begin name=... -->/<!-- agents:end ... --> markers — the
# marker namespace stays "agents:" for backwards compatibility with
...
ALL_SKILLS=()
while IFS= read -r n; do ALL_SKILLS+=("$n"); done < <(all_skill_names)
...
if [[ -z "$INCLUDE" ]]; then
  ...
  for p in "${parts[@]}"; do
    p="${p// /}"
    [[ -n "$p" ]] || continue
    if [[ ! -f "$SKILLS_DIR/$p.md" ]]; then
      echo "unknown skill: $p" >&2
      echo "available: $(all_skill_names | tr '\n' ' ')" >&2
      exit 2
    fi
    RESOLVED+=("$p")
  done
...
if [[ ${#RESOLVED[@]} -eq 0 ]]; then
  echo "no skills resolved from --include" >&2
  exit 2
fi
...
build_banner() {
  cat <<'EOF'
<!-- agents:banner:begin -->
> Auto-generated by `bin/install-skills.sh` from the skill source repo. Do
> not hand-edit this file — edit the source skill and re-run
> `install-skills.sh`.
>
...
# Build a TOC from a list of skill names read on stdin (one per line).
build_toc_from_list() {
...
`AGENTS.md`. Edit freely — `install-skills.sh` never touches this file.
...
run_override() {
  ...
  printf '%s\n' "$content" >"$AGENTS_MD"
  echo "wrote $AGENTS_MD (override, ${#RESOLVED[@]} skills)" >&2
}
...
run_append() {
  ...
  if [[ ${#to_add[@]} -eq 0 ]]; then
    echo "all requested skills already present — nothing to do" >&2
    return 0
  fi
  ...
  echo "appended ${#to_add[@]} skill(s): ${to_add[*]}" >&2
}
...
run_update() {
  ...
  if ! grep -q '<!-- agents:begin name=' "$AGENTS_MD"; then
    echo "AGENTS.md exists but contains no skill markers." >&2
    echo "Refusing to update a hand-written file. Use --mode override" >&2
    echo "to replace it, or --mode append to add skills alongside." >&2
    exit 2
  fi
  ...
  echo "updated $AGENTS_MD (${#RESOLVED[@]} skill(s) processed)" >&2
}
