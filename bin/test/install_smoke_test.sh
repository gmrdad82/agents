...
#
# Creates a throwaway fake project dir, exercises install-skills.sh /
# diff-skills.sh / check-skills.sh through their main code paths, and
# asserts the expected outcomes. Safe — never touches HOME or any real
# project.
...
INSTALL="$REPO_ROOT/bin/install-skills.sh"
DIFF="$REPO_ROOT/bin/diff-skills.sh"
CHECK="$REPO_ROOT/bin/check-skills.sh"
...
# ---- 3: check-skills.sh enumerates correctly ----
step "check-skills.sh enumerates"
got="$("$CHECK" "$tmp" | tr '\n' ',' | sed 's/,$//')"
[[ "$got" == "architect,rails,reviewer" ]] \
  || die "check-skills.sh returned '$got' (expected architect,rails,reviewer)"

# ---- 4: diff-skills.sh reports missing for the rest ----
step "diff-skills.sh reports missing"
if "$DIFF" "$tmp" >/dev/null 2>&1; then
  die "diff-skills.sh exited 0 but skills are missing"
fi

# ---- 5: append adds one more skill without touching others ----
...
# ---- 6: install all skills so diff-skills.sh has a fair starting point ----
step "install all skills (override) so we can test stale detection"
"$INSTALL" "$tmp" --mode override 2>/dev/null
"$DIFF" "$tmp" >/dev/null 2>&1 \
  || die "diff-skills.sh not in-sync after fresh full override"

# ---- 7: a body mutation alone does NOT trigger stale (sha tracks source) ----
step "body mutation is invisible to diff-skills.sh (sha tracks source file)"
sed -i 's|## Conventions|## Conventions (MUTATED)|' "$tmp/AGENTS.md"
"$DIFF" "$tmp" >/dev/null 2>&1 \
  || die "diff-skills.sh wrongly reports drift after body-only mutation"
...
if "$DIFF" "$tmp" >/dev/null 2>&1; then
  die "diff-skills.sh missed stale rails (fake sha)"
fi
...
"$DIFF" "$tmp" >/dev/null 2>&1 \
  || die "diff-skills.sh still reports drift after update"
